...
async function addContact(data, token) {
  ...
  const promise = await axios.post(`${process.env.REACT_APP_API}/addContact`, data, config);
  return promise;
}
...
async function deleteContact(id, token) {
  ...
  const promise = await axios.delete(`${process.env.REACT_APP_API}/home/${id}`, config);
  return promise;
}

async function updateContact(body, id) {
  const promise = await axios.put(`${process.env.REACT_APP_API}/home/${id}`, body);
  return promise;
}
